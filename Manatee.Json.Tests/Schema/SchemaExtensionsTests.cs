        ...
        private class QuasiSchema7IfThenElseValidator : IJsonSchemaPropertyValidator
        {
            ...
            public SchemaValidationResults Validate(IJsonSchema schema, JsonValue json, JsonValue root)
            {
                ...
                if (ifResults.Valid)
                {
                    ...
                    return new SchemaValidationResults(schema, "then", "Validation of `if` succeeded, but validation of `then` failed.");
                }

                var elseResults = _ValidateSubSchema(typed.Else, json, root);
                if (elseResults.Valid) return new SchemaValidationResults();

                return new SchemaValidationResults(schema, "else", "Validation of `if` failed, but validation of `else` also failed.");
            }
